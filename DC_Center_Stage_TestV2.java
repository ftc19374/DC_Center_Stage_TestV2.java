    ...
    public void runOpMode() {
        ...
        while(opModeIsActive()) {
            ...
            if (gamepad2.left_trigger >= 0.5 && Lift_Motor.getCurrentPosition() > 0) {
                //left trigger on gamepad 2 moves the lift up & includes position statements to prevent lift from going too far up
                LPower = 1;
                Lift_Motor.setPower(LPower);
            } else if (gamepad2.right_trigger >= 0.5 && Lift_Motor.getCurrentPosition() <= 2950) {
                //right trigger on gamepad 2 moves lift down
            LPower = -1;
            Lift_Motor.setPower(LPower);
            }else{
